...
private:
    ...
    void generateGetVectors() 
    {
	    ...
	    if (tr_->checkBranch("ResolvedTopCandidate")){
	    ...
	    // only apply JER systematics to MC, not Data
	    registerTLV("Jet", "_jerUp");      // jec up (for systematics)
            registerTLV("Jet", "_jerDown");    // jec down (for systematics)
	    //if (tr_->checkBranch("ResolvedTopCandidate")){
	    //  registerTLV("ResolvedTopCandidate_JERUp");      // resolved tops JER up
	    //  registerTLV("ResolvedTopCandidate_JERDown");    // resolved tops JER down
	    //}
        }
    ...
    void registerTLV(const std::string& objectName, const std::string& jecTag = "")
    {
        ...
        // get pt, eta, phi, mass
        const auto& vec_pt   = tr_->getVec<float>(objectName + "_pt" + jecTag);
        const auto& vec_eta  = tr_->getVec<float>(objectName + "_eta");
        const auto& vec_phi  = tr_->getVec<float>(objectName + "_phi");
        const auto& vec_mass = tr_->getVec<float>(objectName + "_mass" + jecTag);
        ...
        tr_->registerDerivedVec(objectName + "TLV" + jecTag, VectorTLV);
    }
