...
float BaselineVessel::CalculateHT(const std::vector<TLorentzVector>& jetsLVec)
{
    float HT = 0.0;
    for(const auto& jet : jetsLVec)
    {
        if(selJetsForCount(jet)) HT += jet.Pt();
    }
    return HT;
}

bool BaselineVessel::selEle(const TLorentzVector& elecLV, const int elecCutBasedIDNoIso, const float miniPFRelIso_all)
{
    if(fabs(elecLV.Eta()) > 2.5 || elecLV.Pt() < 5) return false;
    
    // Veto ID electron
    if(elecCutBasedIDNoIso < 1) return false;
       
    // MiniIso < 0.1
    if(miniPFRelIso_all > 0.1) return false;
    
    return true;
}

bool BaselineVessel::selMuon(const TLorentzVector& muonLV, const int muonCutBasedIDNoIso, const float miniPFRelIso_all)
{
    // NanoAOD store loose ID Muon by default
    if(fabs(muonLV.Eta()) > 2.4 || muonLV.Pt() < 5) return false;

    // MiniIso < 0.1
    if(miniPFRelIso_all > 0.2) return false;
    
    return true;
}

bool BaselineVessel::selIsotrack(const TLorentzVector& isoTrkLV, const float isk_pfRelIso03_chg, const int isoTrkPdgId, const TLorentzVector& met)
{
    if(abs(isoTrkPdgId) == 11 || abs(isoTrkPdgId) == 13)
    {
        if(isoTrkLV.Pt() < 5 || isk_pfRelIso03_chg > 0.2) return false;
    }
    if(abs(isoTrkPdgId) == 211)
    {
        if(isoTrkLV.Pt() < 10 || isk_pfRelIso03_chg > 0.1) return False;

        float mtW = sqrt( 2 * met.Pt() * isoTrkLV.Pt() * (1 - cos(met.Phi()-isoTrkLV.Phi())));

        if(mtW  > 100) return false;
 
        return True;
    }
}

bool BaselineVessel::selBtagJets(self, const double jetBTagDisc)
{
    if(jetBTagDisc < DeepCSVMediumWP[self.era]) return false;
       
    return true;
}

bool BaselineVessel::selSoftb(const int isv_JetIdx, const int isv_ntracks, const int isv_dlenSig, const float isv_DdotP, const std::vector<unsigned char>& Jet_Stop0l)
{
    // Select soft bs
    // SV is not associate with selected jets
    if(isv_JetIdx >0 && isv_JetIdx < len(Jet_Stop0l) && Jet_Stop0l[isv_JetIdx]) return false;

    if(isv.ntracks < 3 || fabs(isv.dxy)>3. || isv.dlenSig <4) return false;

    if(isv_DdotP < 0.98) return false;

    return true;
}

bool BaselineVessel::selPhotons(const TLorentzVector& photonLV, const int cutBasedBitmap)
{
    if(photonLV.Pt() < 200) return false;
    
    float& abeta = math.fabs(photon.eta);
                               
    if( (abeta > 1.442 && abeta < 1.566) || (abeta > 2.5) ) return false;

    // cut-base ID, 2^0 loose ID
    if(!(cutBasedBitmap & 0x1)) return false;
...
bool BaselineVessel::passEventFilter()
{
    ...
    {
        // Common filters
        passEventFilter = tr->getVar<bool>("goodVertices") && tr->getVar<bool>("HBHENoiseFilter") && 
            tr->getVar<bool>("HBHENoiseIsoFilter") && tr->getVar<bool>("EcalDeadCellTriggerPrimitiveFilter") 
            && tr->getVar<bool>("BadPFMuonFilter") && tr->getVar<bool>("BadChargedC&&idateFilter"); // Post 2016 ICHEP
            // Only data
                
        if(isData_)
        {
            passEventFilter = passEventFilter && tr->getVar<bool>("globalSuperTightHalo2016Filter") && tr->getVar<bool>("eeBadScFilter");
        }
        else if(!isFastSim_)
        {
            passEventFilter = passEventFilter && tr->getVar<bool>("globalSuperTightHalo2016Filter");
        }
    }
    //~~~~~ 2017 ~~~~~
    else if(era_ == ERA_2017 || era_ == ERA_2017)
    {
        // Common filters
        passEventFilter = tr->getVar<bool>("goodVertices") && tr->getVar<bool>("HBHENoiseFilter") && 
            tr->getVar<bool>("HBHENoiseIsoFilter") && tr->getVar<bool>("EcalDeadCellTriggerPrimitiveFilter") 
            && tr->getVar<bool>("BadPFMuonFilter") && tr->getVar<bool>("BadChargedC&&idateFilter") 
            && tr->getVar<bool>("ecalBadCalibFilter")  // Need to double check whether is outdated
            //Only data
            if(isData_)
            {
                passEventFilter = passEventFilter && tr->getVar<bool>("globalSuperTightHalo2016Filter") && tr->getVar<bool>("eeBadScFilter");
    ...
    {
        THROW_SATEXCEPTION("Era not defined!!!");
    }

    return passEventFilter
}

bool BaselineVessel::passJetID(const std::vector<TLorentzVector>& jetsLVec, const std::vector<int>& jetId)
{
    ...
    std::vector<int> jetIDs(jetId_orig.size(), 0);
    for(int i = 0; i < jetId_orig.size(); ++i)
    {
        if(jetsLVec[i].Pt() > 30 && !(jetId[i] & 0x2))
        {
            return false
        }
    }
...
bool BaselineVessel::selJetsForCount(const TLorentzVector& jet)
{
    if(jet.Pt() < 20 or fabs(jet.Eta()) > 2.4) return false;

    return true;
}

int BaselineVessel::calcNjets(const std::vector<TLorentzVector>& jetsLVec)
{
    int nJets = 0;
    for(const auto& jet : jetsLVec)
    {
        if(selJetsForCount(jet)) ++nJets;
    }
    return nJets;
}

std::vector<float> BaselineVessel::calcDeltaPhi(int nDPhi, const std::vector<TLorentzVector>& jetsLVec, const TLorentzVector& met)
{
    ...
    for(const auto& jet : jetsLVec)
    {
        ...
        {
            deltaPhis.push_back(ROOT::Math::VectorUtils::DeltaPhi(jet, met));
            ++iDeltaPhi;
        }
        if(iDeltaPhi >= nDPhi) break;
    }
    return deltaPhis;
}
...
void BaselineVessel::triggerInfo(NTupleReader& tr)
{
