...
parser.add_option ('-b',  dest='bDisc',                 type='float'       , default = -0.5,  help="b tagging discriminator")
...
filestoTransferTT  = [environ["CMSSW_BASE"] + "/src/SusyAnaTools/Tools/bTagEfficiencyCalc",
...
Executable = $ENV(CMSSW_BASE)/src/SusyAnaTools/Tools/condor/goBTagEff.sh
Requirements = OpSys == "LINUX"&& (Arch != "DUMMY" )
Should_Transfer_Files = YES
WhenToTransferOutput = ON_EXIT
Transfer_Input_Files = $ENV(CMSSW_BASE)/src/SusyAnaTools/Tools/condor/goBTagEff.sh,bTagEff.tar.gz,$ENV(CMSSW_VERSION).tar.gz
Output = logs/bTagEff_$(Process).stdout
Error = logs/bTagEff_$(Process).stderr
Log = logs/bTagEff_$(Process).log
...
sc = SampleSet(options.sampleSetCfg)
datasets = []
...
exeName = "bTagEfficiencyCalc"
makeExeAndFriendsTarrball(filestoTransferTT, "bTagEff")

nFilesPerJob = options.numfile

for n, s in sc.sampleSetList():
    ...
    if not f == None:
        ...
        for startFileNum in xrange(0, count, nFilesPerJob):
            fileParts.append("Arguments = %s $ENV(CMSSW_VERSION) %i %i %f %s %s\n"%(n, nFilesPerJob, startFileNum, options.bDisc, s, options.sampleSetCfg.split("/")[-1]))
